...
namespace larcv3 
{
	PSet::PSet(const std::string name, const std::string data)
	{
		if(name.empty()) 
		{
			/// TODO: Consider using spdlog for logging messages
			/// since this avoids things like "std::endl"
	...
	std::pair<PSet::KeyChar_t,size_t> PSet::search(const std::string& txt, 
	...
	{
		/// TODO: Output statements should be sent to the logger
	...
	void PSet::add(const std::string& contents)
	{
		...
		if(end_index <= index || end_index > contents.size()) {
			return;
		}
		...
		while(index <= end_index) 
		{
			...
			if(next_marker.second > end_index) {
				break;
			}
			if(next_marker.first == kNone) {
				break;
			}
			if(next_marker.first == kParamDef) 
			{	
				/// TODO: No else in this block
...
{
	pybind11::class_<larcv3::PSet> pset(m, "PSet");
	pset.def(pybind11::init<const std::string &, const std::string &>(),
			 pybind11::arg("name")="",
			 pybind11::arg("data")="");
	pset.def(pybind11::init<const larcv3::PSet &>(),
			 pybind11::arg("orig"));

	pset.def(pybind11::self == pybind11::self);
	pset.def(pybind11::self != pybind11::self);
	pset.def("__getitem__",  &larcv3::PSet::operator[], pybind11::is_operator());
	pset.def("__setitem__",  &larcv3::PSet::update);

	pset.def("rename",         &larcv3::PSet::rename);
	pset.def("name",           &larcv3::PSet::name);
	pset.def("clear",          &larcv3::PSet::clear);
	pset.def("add",            &larcv3::PSet::add);
	pset.def("add_pset",       (void (larcv3::PSet::*)(const larcv3::PSet& p))(&larcv3::PSet::add_pset));
	pset.def("add_value",      &larcv3::PSet::add_value);
	pset.def("dump",           &larcv3::PSet::dump, pybind11::arg("indent_size")=0);
	pset.def("__repr__",       &larcv3::PSet::dump, pybind11::arg("indent_size")=0);
	pset.def("data_string",    &larcv3::PSet::data_string);
	pset.def("get_pset",       &larcv3::PSet::get_pset);

	pset.def("size",           &larcv3::PSet::size);
	pset.def("keys",           &larcv3::PSet::keys);
	pset.def("value_keys",     &larcv3::PSet::value_keys);
	pset.def("pset_keys",      &larcv3::PSet::pset_keys);
	pset.def("contains_value", &larcv3::PSet::contains_value);
	pset.def("contains_pset",  &larcv3::PSet::contains_pset);
}