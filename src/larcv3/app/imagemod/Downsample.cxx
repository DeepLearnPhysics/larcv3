...
void Downsample::configure(const PSet& cfg) {
  ...
  if (_thresholds_v.empty()) {
    auto Downsample = cfg.get<float>("Downsample");
    _thresholds_v.push_back(Downsample);
  }
...
bool Downsample::process(IOManager& mgr) {
  ...
  for (size_t producer_index = 0; producer_index < _input_producer_v.size();
    ...
    if (product == "sparse2d"){
      ...
      for (size_t i = 0; i < ev_input.as_vector().size(); i ++ ){
        ...
        if ( _thresholds_v.size() == 1){
          Downsample = _thresholds_v[0];
        }
        else if(i < _thresholds_v.size() ){
          Downsample = _thresholds_v[i];
        }
        else{
    ...
    // auto const& Downsample = _thresholds_v[producer_index];
