...
namespace larcv3 
{
	 ...
	 * @detail Inherited class must implment 4 pure virtual functions that are 
	 * called by larcv3::ProcessDriver instance.
	 * 
	 * ProcessBase::configure(const larcv3::PSet) is called first with the argument 
	 * passing the configuration parameters.
	 * 
	 * ProcessBase::initialize() is called after configure. This is where you may 
	 * want to initialize variables.
	 * 
	 * ProcessBase::process(larcv3::IOManager&) is called for every event. The 
	 * argument provides an access to event data.
	 * 
	 * ProcessBase::finalize() is called after larcv3::ProcessDriver finished 
	...
	*/
	...
	public:
		...
		/// Four pure virtual functions that larcv3::ProcessDriver calls 
		/// and need implementation.
		/// Called first with the argument passing the configuration parameters.
		virtual void configure(const PSet&) = 0;

		/// Called after configure, this is where you should initialize variables 
		/// to be stored in an output analysis file.
		virtual void initialize() = 0;

		/// Called per-event, this is where you should implement your per-event 
		/// action/analysis.
		virtual bool process(IOManager& mgr) = 0;

		/// Called after event loop is over. This is where you can store your 
		/// histograms etc. to an output analysis file.
		virtual void finalize() = 0;

		/// Only for experts: allows a loose grouping for a set of 
		/// ProcessBase inherit classes via true/false return to a "question".
		virtual bool is(const std::string question) const;

		/// Only for experts: larcv3::ProcessDriver to see if this 
		/// module can create a new event or not
