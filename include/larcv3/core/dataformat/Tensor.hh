...
namespace larcv3 
{
	...
	/**
	 * @brief Meant to be a storage class for a tensor of arbitrary dimension 
	 * (originally a 2D image). Ultimately data is 1D array.
	 */
	template<size_t dimension>
	class Tensor 
	{
	...
	public:
		...
		/// constructor from ImageMeta
		Tensor(const ImageMeta<dimension>&);

		/// constructor from ImageMeta and 1D array data
		...
		const ImageMeta<dimension>& meta() const 
		{ return _meta; }

		/// Mem-copy: insert num_pixel many data from src 1D array @ data 
		/// index starting from (row,col)
		void copy(const std::vector<size_t> & coords, 
			const float* src, size_t num_pixel);

		/// Mem-copy: insert num_pixel many data from src 1D array @ data 
		/// index starting from (row,col)
		void copy(const std::vector<size_t> & coords, 
			const std::vector<float>& src, size_t num_pixel=0);

		/// Fake mem-copy (loop element-wise copy): insert num_pixel many data 
		/// from src 1D array @ data index starting from (row,col)
		void copy(const std::vector<size_t> & coords, 
			const short* src, size_t num_pixel);

		/// Fake mem-copy (loop element-wise copy): insert num_pixel many data 
		///	from src 1D array @ data index starting from (row,col)
		void copy(const std::vector<size_t> & coords, 
			const std::vector<short>& src, size_t num_pixel=0);

		/// Same as copy, but perform in reverse direction of rows 
		/// (useful when src is not in the same order)
		void reverse_copy(const std::vector<size_t> & coords, 
			const std::vector<float>& src, size_t nskip=0, size_t num_pixel=0);

		/// Same as copy, but perform in reverse direction of rows 
		/// (useful when src is not in the same order)
		void reverse_copy(const std::vector<size_t> & coords, 
			const std::vector<short>& src, size_t nskip=0, size_t num_pixel=0);
		...
		Tensor<dimension> compress(std::array<size_t, dimension> compression, 
			PoolType_t) const;
		...
		// Implementations are here in the source code until a later cleanup, 
		// in case they need to be un deprecated:
		...
		void reset_origin(double x, double y) 
		{_meta.reset_origin(x,y);}

		/// Compress image2D data and returns compressed data 1D array
		std::vector<float> copy_compress(size_t row_count, size_t col_count, 
			CompressionModes_t mode=kSum) const;
	...
	/// TODO: Why is this one called Image2D?
	using Image2D = Tensor<2>;
