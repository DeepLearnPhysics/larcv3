...
namespace larcv3 
{
	...
	public:
		...
		/// Value setter
		inline void set(VoxelID_t id, float value) 
		{ _id = id; _value = value; }
	...
	/**
	 * @brief Container of multiple voxels consisting of 
	 * ordered sparse vector and meta data
	 */
	class VoxelSet 
	{
	public:
		...
		/// InstanceID_t getter
		inline InstanceID_t id() const 
		{ return _id; }

		/// Access as a raw vector
		inline const std::vector<larcv3::Voxel>& as_vector() const 
		{ return _voxel_v; }

		/// Returns a const reference to a voxel with specified id. 
		/// if not present, invalid voxel is returned.
		...
		{ return (_voxel_v.empty() ? 0. : sum() / (float)(_voxel_v.size())); }
		...
		/// Add a new voxel. If another voxel instance w/ same 
		/// VoxelID exists, value is added
		void add(const Voxel& vox);

		/// Set a voxel. If another voxel instance w/ same VoxelID exists, 
		/// value is set
		void insert(const Voxel& vox);

		/// Emplace a new voxel. Same logic as VoxelSet::add but consumes 
		/// removable reference.
		...
		/// InstanceID_t setter
		inline void id(const InstanceID_t id) 
		{ _id = id; }
    	...
    	std::vector<larcv3::Voxel> & writeable_voxel_vector()
		{return _voxel_v;}
	...
	/**
	 * @brief Container of multiple VoxelSet 
	 * (i.e. container w/ InstanceID_t & VoxelSet pairs)
	 */
	class VoxelSetArray 
	{
	public:
	...
	public:
		...
		inline const larcv3::ImageMeta<dimension>& meta() const 
		{ return _meta; }
		...
		SparseTensor<dimension> compress(std::array<size_t, dimension> compression, 
			PoolType_t) const;
		...
		{ VoxelSet::clear_data(); _meta = ImageMeta<dimension>(); }
	...
	public:
		...
		/// Clear everything
		inline void clear_data() 
		{ VoxelSetArray::clear_data(); _meta = ImageMeta<dimension>(); }
	...
	using SparseTensor2D = SparseTensor<2>;
	using SparseTensor3D = SparseTensor<3>;
	using SparseCluster2D = SparseCluster<2>;
	using SparseCluster3D = SparseCluster<3>;
}
