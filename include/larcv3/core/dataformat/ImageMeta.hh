...
{
	/**
	 * @brief This class defines an abstract rectangular voxelized region 
	 * in N dimension space. The class is represented by it's number of 
	 * dimensions, as well as the number of voxels in each dimension and the 
	 * absolute length of each dimension in real space. A "position" is the 
	 * real valued coordinate inside this space. A "coordinate" is the 
	 * voxelized location inside this space, ranging from 0 to n_dims. 
	...
	public:
		...
		inline bool operator==(const ImageMeta<dimension> & rhs) const 
		{
			...
			return (_projection_id == rhs._projection_id &&
					_unit == rhs._unit);
		}
		...
		inline const size_t projection_id() const
		...
		inline const double * image_size()       const 
		{ return _image_sizes; }

		inline const size_t * number_of_voxels() const 
		{ return _number_of_voxels; }

		inline const double * origin()           const 
		{ return _origin; }

		std::vector< size_t > strides() const;

		inline size_t n_dims()  const 
		{ return dimension; }
		...
		/// 2D length unit
		inline DistanceUnit_t unit() const 
		{ return _unit; }
		...
		/// Coordinates is flattened here as 
		/// [x0, y0, z0, x1, y1, z1, x2, y2, z2, x3 ....]
		void index(const std::vector<size_t>& coordinates, 
		...
		/// Coordinates is flattened here as 
		/// [x0, y0, z0, x1, y1, z1, x2, y2, z2, x3 ....]
		void coordinates(const std::vector<size_t> & index,  
			std::vector<size_t>& output_coordinates) const;

		/// Convert 1D index to overall coordiante along specified axis
		size_t coordinate(size_t index, size_t axis) const;
		/// There is no vectorized version of the single axis coordinate, 
		...
		void set_dimension(size_t axis, double image_size, 
			size_t number_of_voxels, double origin = 0);

		inline void set_projection_id(size_t projection_id)
	...
	protected:
		/// Associated image ID (of the same producer name)
		// ImageIndex_t _image_id;  

		/// Boolean set to true only if voxel parameters are properly set
		bool _valid;  
	...
	using ImageMeta1D = ImageMeta<1>;
	using ImageMeta2D = ImageMeta<2>;
	using ImageMeta3D = ImageMeta<3>;
	using ImageMeta4D = ImageMeta<4>;
}
