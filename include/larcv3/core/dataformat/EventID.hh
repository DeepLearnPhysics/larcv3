...
namespace larcv3 
{
	 ...
	 * (what is stored in output file), holding
	...
	public:
		...
		/// Run number setter
		inline void run(long _run) 
		{ this->_run = _run; }

		/// SubRun number setter
		inline void subrun(long _subrun) 
		{ this->_subrun = _subrun; }

		/// Event number setter
		inline void event(long _event) 
		{ this->_event = _event; }

		/// Make sure run/subrun/event ID is set
		bool valid() const 
		{
			return !(_run == kINVALID_LONG || 
					 _subrun == kINVALID_LONG ||
