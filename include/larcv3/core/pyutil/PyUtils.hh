...
/// TODO: fix compiler definitions here
//#ifndef __CLING__
//#ifndef __CINT__
#include <Python.h>
...
namespace larcv3 
{
	...
	// void fill_3d_pcloud(const SparseTensor3D &data, 
	//	PyObject* pyarray, PyObject* select=nullptr);

	// /// larcv3::VoxelSet to 2D numpy point cloud array (NUM_POINTS,1/3/4) converter
	// void fill_3d_voxels(const SparseTensor3D &data, 
	//	PyObject* pyarray, PyObject* select=nullptr);

	// /// larcv3::VoxelSet to 2D numpy point cloud array (NUM_POINTS,1/2/3) converter
	// void fill_2d_pcloud(const SparseTensor2D &data, 
	//	PyObject* pyarray, PyObject* select=nullptr);

	// /// larcv3::VoxelSet to 2D numpy point cloud array (NUM_POINTS,1/2/3) converter
	// void fill_2d_voxels(const SparseTensor2D &data, 
	//	PyObject* pyarray, PyObject* select=nullptr);
	...
	// inline VoxelSet as_tensor3d(const EventSparseTensor3D& vs, 
	//	PyObject* val_array, float min_threshold=0)
	// { return as_tensor3d( (const SparseTensor3D)vs, val_array, min_threshold); }

	// // allows one to avoid some loops in python
	// void fill_img_col(Image2D &img, std::vector<short> &adcs, 
	//	const int col,const float pedestal = 0.0);
